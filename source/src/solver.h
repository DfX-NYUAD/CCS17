...
private:
    ...
    bool _solve_v0(std::map<std::string, int>& keysFound, bool quiet, int dlimFactor);
    void _testBackbones(
        const std::vector<bool>& inputs, 
        sat_n::Solver& S, ckt_n::index2lit_map_t& lmap,
        std::map<int, int>& backbones);
    void _queryOracle(); // added for querying the oracle  
public:
    // flags and limits.
    int verbose;
    double time_limit;
    ...
    bool solve(solver_version_t ver, std::map<std::string, int>& keysFoundMap, bool quiet);
    void findFixedKeys(std::map<int, int>& backbones);

    static void solveSlice(
        slice_t& slice, 
        std::map<int, int>& fixedKeys,
        std::map<std::string, int>& allKeys 
    );
    int sliceAndSolve( std::map<std::string, int>& keysFoundMap, int maxKeys, int maxNodes );
};
