...
int sld_main(int argc, char* argv[]) 
{
    ...
    while ((c = getopt (argc, argv, "ihvptTc:m:k:s")) != -1) {
        switch (c) {
            case 'h':
    ...
    if(yyparse() == 0) {
        ...
        // Commented out as there is no need for simckt when Oracle is there
	yyin = fopen(argv[optind+1], "rt");
        ...
        ckt_n::ckt_t simckt(*statements, false );
        delete statements;
        fclose(yyin);
	...
	// commented as not required in presence of Oracle 
